...
public class JsonDeserializer_old implements Deserializer {

    private static final Logger LOGGER = LoggerFactory.getLogger(JsonDeserializer_old.class);
    ...
    public <T> T deserialize(String input, Typed<T> typed) throws DeserializationException {
        return deserialize(new JsonCharArrayReader(input), typed);
    }

    public <O> O deserialize(JsonReader reader, Typed<O> typed) throws DeserializationException {
        if(LOGGER.isDebugEnabled()) {
            LOGGER.debug("deserialize(reader <{}>, type <{}>)", reader, typed);
        }
        O result = null;
